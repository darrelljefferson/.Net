...
namespace RailEuropeFileParser
{
    class CsvParser
    {
        public static void ParseAndCreateNewCsvFile(string workingFile,
             string newFileName, string triggerID,  Boolean hasOrderID)
        {
            ...
            using (StreamWriter file = new StreamWriter(newFileName))
            {
                ...
                while (reader.EndOfStream == false)
                {
                    ...
                    // only add rows if the email quailify for email blast into the Database

                    string wf = StripDownPath(workingFile);

                    if (HistoryTracker.DetermineIfEmailToSend(value[0], wf ))  // If return prep to send email
                    {

                        // check to see if the value is unknown and overwrite
                        if (value[1].Replace("\"", "").ToLower() == "unknown" || value[1].Replace("\"", "").ToLower() == "undefined")
                        {
                            value[1] = "";
                        }
                        ...
                        if (hasOrderID == true && isValidEmail == true)
                        {
                            ...
                            // build the line
                            csvLine = string.Concat(value[0].Replace("\"", ""), seperator, value[2].Replace("\"", ""));
                        ...
                        {
                            // build the line
                            csvLine = string.Concat(value[0].Replace("\"", ""), seperator, value[1].Replace("\"", ""));
                        }

                        // write the line to file
                        file.WriteLine(csvLine);

                        // add the email & datafile to the MS Sql Database
                        HistoryTracker.AddEmailAddr(value[0], workingFile);
        ...
        public static string StripDownPath(string wf)
        {
            string purefilename = wf.Remove(1, 10);
            return wf;
            ...
            string ftpFilePath)
        {
            ...
            while (reader.EndOfStream == false)
            {
                ...
                SendEmail.CreateEmailMember(siteid, mlid, value[0], value, apiPassword);

                // if we don't care about the file, then just send messages one at a time
                if (hasMultipleEmails == false)
                {
                    SendEmail.CreateEmailMember(siteid, mlid, value[0], value, apiPassword);  // dynamically make new member ok if already there
                    SendEmail.SendSingeRecipientEmails(siteid, mlid, triggerid, value[0], value, apiPassword);   // was commented out asof 6/28/2011
                        
                   
                    //ReceiptsManager.AddReceipt(value[0]);
                }
            ...
            if (hasMultipleEmails == true)
            {
                ...
                //------ Version 1 ----- SEND BIG SINGLE REQUEST
                //SendEmail.SendEmailsPostRequest(siteid, mlid, triggerid, recipients,
                //    ftpFilePath, apiPassword);


                //------ Version 2 ----- SEND MULTIPLE REQUESTS IN CHUNKS....  
                ...
                foreach (List<string> chunk in chunks)
                {
                    ...
                    SendEmail.SendEmailsPostRequest(siteid, mlid, triggerid, currentRecipients, ftpFilePath, apiPassword);
                    // ReceiptsManager.AddReceipt(currentRecipients);
                }
            }
        }
