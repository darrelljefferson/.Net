...
namespace PheaaUploadAndFireTriggerParser
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //DateTime today = Convert.ToDateTime("2011-06-02");
            ...
            foreach (KeyValuePair<string, string> item in listMlidMap)
            {
               ...
               // string notifyEmail = "bmueller@lyris.com";
                ...
                //string notifyEmail = "djefferson@lyris.com";
            ...
            string file, string password)
        {
            ...
            try
            {
                ...
                DetermineifCompleted( file , siteid,  mlid,  password);
            }
            catch (Exception e)
            {
        ...
        public static void DetermineifCompleted(string file, string siteid, string mlid, string password)
        {
            ...
            string recordStatus = String.Concat("https://www.elabs7.com/API/",
                "mailing_list.html?",
                "type=record&",
            ...
            else
            {
                ...
                IEnumerable<XElement> status =
                    from sl in uploadStatus.Descendants("DATA")
                    select sl;

                foreach (XElement sl in status)
                {
                    if ((string)sl.Attribute("STATUS").Value != "done")
                    {
                ...
                Log.WriteLine(uploadStatus.Element(dataset).Element(data).Value);
